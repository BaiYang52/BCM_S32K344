...
{
  /*------------------------------------------------------------------------
   * Step 1: Disable the SWT_0 hardware watchdog immediately, BEFORE any
   *         other initialization, to prevent a reset during BSS/DATA copy.
   *         Use the register address directly to avoid dependency on the
   *         S32K344.h macro expansion order.
   *------------------------------------------------------------------------*/
  /* Unlock SWT_0 service register (write sequence) */
  *(volatile uint32_t *)0x402A0010U = 0xC520U;   /* SWT_0->SR */
  *(volatile uint32_t *)0x402A0010U = 0xD928U;   /* SWT_0->SR */
  /* Clear WEN bit (WEN = 0) and keep other bits (debug-halt etc.) sane */
  *(volatile uint32_t *)0x402A0000U = 0xFF00010AU;  /* SWT_0->CR */
