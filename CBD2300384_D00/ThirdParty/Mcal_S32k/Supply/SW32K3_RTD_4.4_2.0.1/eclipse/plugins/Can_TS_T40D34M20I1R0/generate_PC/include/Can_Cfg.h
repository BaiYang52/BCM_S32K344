    ...
    [!IF "ecu:get('Can.CanConfigSet.TimeStampSrcList') != '' and CanTimeStamp/HRTimeStampCapturePoint != 'FLEXCAN_TIMESTAMPCAPTURE_DISABLE'"!]
        ...
        [!ELSEIF "contains($HR_Time_Src, 'STM')"!]
            ...
            [!IF "not(contains($HR_Time_Src, node:value(GptHwIp)) and contains($HR_Time_Src, string($stm_instance)))"!]
                ...
                [!ENDERROR!]
            [!ENDIF!]
            #define CAN_TS_GPT_CHANNEL_ID ([!"num:i(GptChannelId)"!]U)
            #define CAN_TS_STM_INSTANCE ([!"num:i($stm_instance)"!]U)
            #define CAN_TS_STM_CHANNEL  ([!"text:split(node:ref(GptModuleRef)/GptStmChannel, '_')[2]"!]U)
            [!ENDSELECT!]
            #define CAN_TS_STM_TARGETTIME  ([!"num:i($Gpt_Channel_Tick_Frequency)"!]U)
            #define CAN_TS_ACCURACY  ([!"num:i(CanTimeStamp/CanTimeStampRequiredAccuracy * 1000)"!]UL)
...
#define CAN_TS_EXTERNAL_TIMER_USED[!WS "4"!][!IF "$external_timer_used = 1"!](STD_ON)[!ELSE!](STD_OFF)[!ENDIF!]
