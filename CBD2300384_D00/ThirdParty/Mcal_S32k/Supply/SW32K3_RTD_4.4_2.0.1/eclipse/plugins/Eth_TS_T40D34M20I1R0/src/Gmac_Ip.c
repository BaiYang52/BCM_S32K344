...
#if (STD_ON == GMAC_DEV_ERROR_DETECT)
    #include "Devassert.h"
#endif /* (STD_ON == GMAC_DEV_ERROR_DETECT) */ 
...
#if (GMAC_DEV_ERROR_DETECT == STD_ON)
                                     ...
                                     const Gmac_Ip_TxOptionsType * Options)
{
    ...
    if ((uint32)Bd >= (uint32)&ListBd[Base->DMA_TXDESC_RING_LENGTH + 1UL])
    {
        Bd = ListBd;
    }
                                                ...
                                                uint32 NumBuffers)
{
    ...
    else
    {
        ...
        while (Bd != FirstBd)
        {
            ...
            if ((uint32)Bd < (uint32)ListBd)
            {
                Bd = &ListBd[RingLength];
            }
                           ...
                           const Gmac_Ip_BufferType * Buff)
{
    ...
    if ((Bd->Des1 & GMAC_RDES1_TSA_MASK) != 0U)
    {
        CtxtBd = ((uint32)&Bd[1U] >= (uint32)&ListBd[Base->DMA_RXDESC_RING_LENGTH + 1UL])? ListBd : &Bd[1U];
    ...
    if ((uint32)Gmac_apxState[Instance]->RxAllocDesc[Ring] >= (uint32)&ListBd[Base->DMA_RXDESC_RING_LENGTH + 1UL])
    {
