...
#define SRAM_REGION_0_ADDR              (0x20442000UL)
#define SRAM_REGION_0                   (*(volatile uint32*)SRAM_REGION_0_ADDR)
...
boolean CheckXrdcError(void)
{
    ...
    {
        if(gXrdcErrorDetails.ErrorStatus[u32ErrorPort].u32AddError == SRAM_REGION_0_ADDR)
        {
            bReturnCode = TRUE;
...
int main(void)
{
    ...
    /* Try to write at SRAM_REGION_0, this should trigger a Xrdc error */
    SRAM_REGION_0 = PATTERN;

    /* Delay to waiting for that exception handlers occurred*/
    Delay_Rm(0x1000UL);

    /*Check if hardfault interrupt occurred.*/
    if(gXrdcErrorDetected == TRUE)
    {
