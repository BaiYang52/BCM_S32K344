...
namespace Vector.VTT.VsProjectGenerator
{
  class ModuleVTTOs : IModule
  {
    ...
    {
      string osDir = Helper.GetModuleBSWDir(model.Directory.BSW, "VttOs");
      ...
      if (parameterValue != null)
      {
        ...
        /* OS static files: */
        foreach(string file in Helper.GetFiles(osDir, @"^Os.*\.(h|c)"))
        {
          model.Project.AddBswFile(file, Name);
        }

        /* OS files - Generated into the applications source folder: */
        foreach(string file in Helper.GetFiles(templateDir, @"^Os_.*\.(h|c)"))
        {
          model.Project.AddBswFile(file, Name);
        }
        ...
        foreach(string file in Helper.GetFiles(genDataDir, @"^Os.*\.(h|c)"))
        {
          model.Project.AddBswFile(file, Name);
        }


        Regex regexNumberOfCoresOld = new Regex(@"^\s*OS_COREID_COUNT\s*=.*?(\d+)");
        Regex regexNumberOfCores = new Regex(@"^\s*#\s*define\s+OS_CFG_COREPHYSICALID_COUNT.*?(\d+)");
        ...
        Regex regexCoreStartUp = new Regex(@"^\s*#\s*define\s*OS_CFG_HAL_CORE(\d+)_STARTUP\s*OS_HAL_CORESTARTUP_(.*)");
        ...
        using (StreamReader r = new StreamReader(System.IO.Path.Combine(genDataDir, "Os_Core_Cfg.h")))
        {
          while ((line = r.ReadLine()) != null)
          {
        ...
        // Set the startup behavior of each configured OS core.
        // Requires VTT OS version > 2.22.xx
        // Requires CANoeEmuLib version > 2.31.xx
        // Requires VTT version > 2.0.44
        using (StreamReader r = new StreamReader(System.IO.Path.Combine(genDataDir, "Os_Hal_Vtt_Cfg.h")))
        {
          while ((line = r.ReadLine()) != null)
          {
            if ((m = regexCoreStartUp.Match(line)).Success)
            {
              StartupBehavior startUp = StartupBehavior.eDisableStart;
              if(m.Groups[2].Value.Equals("MANUALSTART"))
                startUp = StartupBehavior.eManualStart;
              else if(m.Groups[2].Value.Equals("AUTOSTART"))
                startUp = StartupBehavior.eAutoStart;

              model.CANoeEmu.SetStartupBehavior(Int32.Parse(m.Groups[1].Value), startUp);
            }
          }
        }
    ...
    void ConfigureLegacyVTTOs(IEcucProvider ecuc, IGeneratorModel model)
    {
      ...
      switch(model.Project.Architecture)
      {
        case TargetArchitecture.eWindowsX86:
          // OK, Windows-x86 is supported by TTOs
          break;
        case TargetArchitecture.eWindowsX64:
          throw new Exception("Target Architecture: Windows-x64 is not supported by VTTOs");
        case TargetArchitecture.eLinuxX64:
          throw new Exception("Target Architecture: Linux-x64 is not supported by VTTOs");
        default:
          throw new Exception("Target Architecture: The selected target architecture is not supported by VTTOs");
      }
#endif
      ...
      switch (model.Project.Compiler)
      {
        ...
        case CompilerVersion.eMingw81:
          throw new Exception("Compiler Version: The Mingw-w64 compiler is not supported by VTTOs");
        case CompilerVersion.eClang60:
          throw new Exception("Compiler Version: The Clang compiler is not supported by VTTOs");
#endif
#if (VTT_MinGW122)
