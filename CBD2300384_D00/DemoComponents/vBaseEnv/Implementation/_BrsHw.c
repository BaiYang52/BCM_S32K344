...
/* USERMODE ACCESS
 *
 * According to S32K3XXRM_rev5.pdf -> S32K3xx_REG_PROT_details.xlsx:
 * "Module Note: SIUL PROT_MEM = 8 other modules PROT_MEM = 4"
 * -> PROT_MEM = 4 for Flash Memory Unit (FMU)
 */
#define BRS_FLASH_BASEADDR (0x402EC000u)
#define BRS_ENABLE_FLASH_USERMODE

#define BRS_UAA_BIT (0x00800000u)

#define BRS_GCR_1K_OFFSET (0x900u)
#define BRS_GCR_2K_OFFSET (0x1200u)
#define BRS_GCR_4K_OFFSET (0x2400u)
#define BRS_GCR_8K_OFFSET (0x4800u)
#define BRS_GCR_16K_OFFSET (0x9000u)
#define BRS_GCR_32K_OFFSET (0x12000u)

#warning "Check if BRS_FLASH_GCR_OFFSET is set according to the size of PROT_MEM for the Flash Memory Unit (FMU) on your chip and remove this warning!"
#define BRS_FLASH_GCR_OFFSET BRS_GCR_4K_OFFSET
...
# endif /*BRS_PORT_GROUP_x*/
  ...
  #define BRSHW_MC_RGM_FES_SW_FUNC            (0x20000000uL)
  #define BRSHW_MC_RGM_FES_RESET_MASK         (0x601102D9uL)
  
  #define BRSHW_DCM_GPR_BASE 0x402AC000uL
  #define BRSHW_DCM_GPR_DCMRWP1               BRSHW_IOS(uint32, (BRSHW_DCM_GPR_BASE + 0x400))
...
#if defined (BRS_COMP_GHS) 
extern void _Brs_ExcVect_ALL_START(void);
#endif
 ...
 *********************************************************************************************************************/
#if defined (BRS_COMP_GHS)
#pragma ghs section rodata=".IVTField"
#endif
...
#if defined (BRS_COMP_GHS) 
  .CM7_0_StartAddress = (void *)_Brs_ExcVect_ALL_START,
  .CM7_1_StartAddress = (void *)_Brs_ExcVect_ALL_START,
  .CM7_2_StartAddress = (void *)_Brs_ExcVect_ALL_START,
#endif
 ...
 *********************************************************************************************************************/
 /*Enabling of MSCM clock at beginning of startup code*/
  ...
  /*It was discovered that after triggering a software reset the 
    controller needs some time to enable the clock for MSCM module again*/
  ___asm(nop \n) ___asm(nop \n) ___asm(nop \n) ___asm(nop \n) ___asm(nop \n) 
  ___asm(nop \n) ___asm(nop \n) ___asm(nop \n) ___asm(nop \n) ___asm(nop \n)
...
# if !defined (BRS_PLL_GROUP_C)
  /* Enable System Timer Module 1 */
  MC_ME_PRTN2_COFB1_CLKEN |= (1 << 29);
# endif
...
# if ((BRS_TIMEBASE_CLOCK != 48) && (BRS_TIMEBASE_CLOCK != 120) && (BRS_TIMEBASE_CLOCK != 160) && (BRS_TIMEBASE_CLOCK != 240) && (BRS_TIMEBASE_CLOCK != 320))
  #error "Only 48MHz, 120MHz, 160MHz, 240MHz or 320MHz as TIMEBASE_CLOCK supported by the configured derivative. Please configure a supported value in Cfg5"
# endif

# if ((BRS_PERIPH_CLOCK != 48) && (BRS_PERIPH_CLOCK != 120) && (BRS_PERIPH_CLOCK != 160) && (BRS_PERIPH_CLOCK != 240) && (BRS_PERIPH_CLOCK != 320))
  #error "Only 48MHz, 120MHz, 160MHz, 240MHz or 320MHz as PERIPHERAL_CLOCK supported by the configured derivative. Please configure a supported value equal to TIMEBASE_CLOCK in Cfg5"
# endif
...
# if defined (BRS_DERIVATIVE_GROUP_S32K3X8)
  /* Power down PLL_AUX */
...
#   if (BRS_TIMEBASE_CLOCK == 48)
  CORE_PLL_PLLODIV_0 |= (uint32)(19ul << PLLDIG_PLLiDIV_DIV); /* Divide PLLDV by 20 for PLL_PHI0_CLK = 48MHz */
  CORE_PLL_PLLODIV_1 |= (uint32)(19ul << PLLDIG_PLLiDIV_DIV); /* Divide PLLDV by 20 for PLL_PHI1_CLK = 48MHz */
...
#   elif (BRS_TIMEBASE_CLOCK == 320)
  CORE_PLL_PLLODIV_0 |= (uint32)(2ul << PLLDIG_PLLiDIV_DIV); /* Divide PLLDV by 3 for PLL_PHI0_CLK = 320MHz */
...
#   if (BRS_TIMEBASE_CLOCK == 48)
  CORE_PLL_PLLODIV_0 |= (uint32)(9ul << PLLDIG_PLLiDIV_DIV); /* Divide PLLDV by 10 for PLL_PHI0_CLK = 48MHz */
  CORE_PLL_PLLODIV_1 |= (uint32)(9ul << PLLDIG_PLLiDIV_DIV); /* Divide PLLDV by 10 for PLL_PHI1_CLK = 48MHz */
#   elif (BRS_TIMEBASE_CLOCK == 120)
...
#   if (BRS_TIMEBASE_CLOCK == 48)
  CORE_PLL_PLLODIV_0 |= (uint32)(4ul << PLLDIG_PLLiDIV_DIV); /* Divide PLLDV by 5 for PLL_PHI0_CLK =  48MHz */
...
# else
  ...
  /* if BRS_TIMEBASE_CLOCK ==  48 --> CORE_CLK ==  48MHz */
...
#  elif ((BRS_TIMEBASE_CLOCK == 160) || (BRS_TIMEBASE_CLOCK == 120) || (BRS_TIMEBASE_CLOCK == 48))
...
# if defined (BRS_PLL_GROUP_D)
  ...
  /*CM7_CORE_CLK is driven directly with PLL_PHI0_CLK frequency, therefor no dividing necessary*/
...
# endif
  ...
  /* if BRS_TIMEBASE_CLOCK ==  48 --> AIPS_PLAT_CLK == 48MHz */
...
#  endif
  ...
  /* if BRS_TIMEBASE_CLOCK ==  48 --> AIPS_PLAT_CLK == 24MHz */
...
#  elif (BRS_TIMEBASE_CLOCK == 48)
  MC_CGM_0_MUX_0_DC(2u) |= (1ul << MC_CGM_0_MUX_DC_DIV);
#  endif
  ...
  /* if BRS_TIMEBASE_CLOCK ==  48 --> HSE_CLK ==  48MHz */
...
#  if ((BRS_TIMEBASE_CLOCK == 48) ||(BRS_TIMEBASE_CLOCK == 120))
  /*Divider value is already cleared, nothing to be done here*/
...
#  endif
  ...
  /* 3.3 Request clock switching to clock mux 0. */
  MC_CGM_0_MUX_CSC |= (1ul << MC_CGM_MUX_CSC_CLK_SW);
  ...
  /* 3.8 Request clock switching to clock mux 1. */
...
brsMain_ResetReasonType BrsHwGetResetReasonStartup(void)
{
     ...
     ((uint32)0u == (BRSHW_MC_RGM_DES & BRSHW_MC_RGM_DES_RESET_WO_POR_MASK)))
  {
    ...
    FESRegVal = BRSHW_MC_RGM_FES;
    do {
      BRSHW_MC_RGM_FES  &= FESRegVal;
  ...
  else
  {
    ...
    if (BRSHW_MC_RGM_DES != (uint32)0u)
    {
      DESRegVal = BRSHW_MC_RGM_DES;
      do {
        BRSHW_MC_RGM_DES  &= DESRegVal;
 ...
 */
#if defined (BRS_ENABLE_FLASH_USERMODE)
  /* set UAA for Flash to enable write in UserMode / set HLB=0 */
  *((volatile uint32*)(BRS_FLASH_BASEADDR + BRS_FLASH_GCR_OFFSET)) = BRS_UAA_BIT;
#endif /* BRS_ENABLE_FLASH_SUPPORT*/
}
...
{  
  /*Disable recovery mode for functional resets*/
  if (!(BRSHW_DCM_GPR_DCMRWP1 & (1UL << 22)))
  {
    BRSHW_DCM_GPR_DCMRWP1 |= (1 << 22);
  }

  /*Functional reset via AIRCR register not possible, has to be triggered via MC_RGM module*/
  MC_ME_MODE_CONF |= (1 << 1);  /* Trigger functional reset request */
  MC_ME_MODE_UPD  |= (1 << 0);  /* Initiate mode change */
  MC_ME_CTL_KEY   &= 0x5AF0ul;  /* Write control key */
  MC_ME_CTL_KEY   &= 0xA50Ful;  /* Write inverted control key */
}
